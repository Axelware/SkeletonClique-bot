...
class Tags(commands.Cog):
    ...
    async def tag_create(self, ctx: context.Context, name: converters.TagNameConverter, *, content: converters.TagContentConverter) -> None:
        ...
        **content**: The content of the tag.
        ...
        if len(guild_config.get_user_tags(ctx.author.id)) >= limit and ctx.author.id not in config.OWNER_IDS:
            raise exceptions.EmbedError(
                colour=colours.RED,
                emoji=emojis.CROSS,
                description=f"You already have the maximum of `{limit}` tags."
            )
